...
def product_new():
    if request.method == "POST":
        ...
        existing_product = get_db().execute(
            "SELECT id, category FROM products WHERE LOWER(name)=LOWER(?)", (name,)
        ).fetchone()
        if existing_product:
            flash(
                f"A product named '{name}' already exists under \"{existing_product['category']}\". "
                f"Each product can only belong to one category — please edit the existing product "
                f"instead of creating a duplicate.", "danger"
            )
            return render_template("admin/product_form.html", product=None, categories=_cats)
...
def product_edit(pid):
    ...
    if request.method == "POST":
        ...
        duplicate = get_db().execute(
            "SELECT id, category FROM products WHERE LOWER(name)=LOWER(?) AND id!=?", (name, pid)
        ).fetchone()
        if duplicate:
            _cats = [r["name"] for r in get_db().execute(
                "SELECT name FROM categories WHERE is_active=1 ORDER BY sort_order,name"
            ...
            flash(
                f"A different product named '{name}' already exists under \"{duplicate['category']}\". "
                f"Each product can only belong to one category — please use a different name.", "danger"
            )
