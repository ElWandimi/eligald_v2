...
def sitemap():
    ...
    cats     = db.execute("SELECT name FROM categories WHERE is_active=1 ORDER BY sort_order").fetchall()
    db.close()
    ...
    for c in cats:
        pages.append({"loc": f"{base}/products?category={c['name'].replace(' ','+')}",
...
def serve_uploaded_file(subfolder, filename):
    """
    Serves user-uploaded files (product images, logos) from the persistent
    Railway Volume, since these live outside the app's built-in static folder
    and must survive redeploys.
    """
    from flask import send_from_directory
    safe_subfolder = secure_filename(subfolder)
    safe_filename  = secure_filename(filename)
    directory = os.path.join(UPLOAD_FOLDER, safe_subfolder)
...
with app.app_context():
    try:
        ...
        # One-time: refresh stale/generic stats to accurate, East-Africa-positioned copy
        try:
            _stats_fix_done = _conn.execute(
                "SELECT COUNT(*) FROM activity_log WHERE action='STATS_REGION_REFRESH_V1'"
            ).fetchone()[0]
        except Exception:
            _stats_fix_done = 0
        if not _stats_fix_done:
            _stat_updates = [
                ("Chemicals Supplied", "177+", "East African Countries Served", "8", "fa-globe-africa"),
                ("Clients Served", None, "Clients Across the Region", None, None),
            ]
            try:
                _conn.execute(
                    "UPDATE company_stats SET number='8', label='East African Countries Served', icon='fa-globe-africa' "
                    "WHERE label='Chemicals Supplied'"
                )
                _conn.execute(
                    "UPDATE company_stats SET label='Clients Across the Region' WHERE label='Clients Served'"
                )
                _conn.execute(
                    "INSERT INTO activity_log (username,action,details) VALUES ('system','STATS_REGION_REFRESH_V1','Updated homepage stats to East Africa positioning')"
                )
                _conn.commit()
                print("[STARTUP] Refreshed homepage stats for East Africa positioning")
            except Exception as _stats_err:
                print(f"[STARTUP] Stats refresh warning: {_stats_err}")

        # Runs EVERY startup (idempotent, safe to repeat): removes any cross-category
        # duplicate products, keeping only the earliest-listed category occurrence.
        # This self-heals duplicates that could otherwise creep in over time.
        try:
            _cat_order = [
            ...
            for _row in _all_prods:
                ...
                if _nm not in _by_name:
                    ...
                    # Keep the lower category-priority match; tie-break on lowest id (oldest record)
                    if (_new_pri, _row["id"]) < (_cur_pri, _cur["id"]):
                        _by_name[_nm] = _row
            ...
            if _removed:
                try:
                    _conn.execute(
                        "INSERT INTO activity_log (username,action,details) VALUES ('system','PRODUCT_DEDUP_AUTO',?)",
                        (f"Auto-removed {_removed} cross-category duplicate product(s) on startup",)
                    )
                except Exception:
                    pass
                _conn.commit()
                print(f"[STARTUP] Auto-deduplication removed {_removed} duplicate product(s)")
        except Exception as _dedup_err:
            print(f"[STARTUP] Product dedup warning: {_dedup_err}")

        # One-time: seed honest About Us / Mission / Vision copy if fields are still empty
        try:
            _copy_row = _conn.execute(
                "SELECT about_us_text, mission_text, vision_text FROM site_settings WHERE id=1"
            ).fetchone()
            if _copy_row and not (_copy_row[0] or "").strip():
                _conn.execute(
                    "UPDATE site_settings SET about_us_text=? WHERE id=1",
                    ("Eligald Industrial Chemicals Limited is a Nairobi-based supplier of industrial "
                     "solvents, acids, bases, and specialty chemicals, serving manufacturers, processors, "
                     "laboratories, and water-treatment authorities across Kenya and the wider East African "
                     "Community.\n\nFrom our warehouse in Imara Daima, we supply a curated range spanning 16 "
                     "product categories. Every product we handle is sourced from vetted manufacturers, with "
                     "full Safety Data Sheets and Certificates of Analysis available on request.\n\nWe combine "
                     "technical know-how with dependable logistics — helping businesses across Kenya, Uganda, "
                     "Tanzania, Rwanda, Burundi, South Sudan, DR Congo, and Somalia get the right chemicals, "
                     "correctly documented, when they need them.",)
                )
            if _copy_row and not (_copy_row[1] or "").strip():
                _conn.execute(
                    "UPDATE site_settings SET mission_text=? WHERE id=1",
                    ("To supply East African businesses with correctly documented, quality-assured "
                     "industrial chemicals — delivered reliably, with the technical guidance needed to use "
                     "them safely and effectively.",)
                )
            if _copy_row and not (_copy_row[2] or "").strip():
                _conn.execute(
                    "UPDATE site_settings SET vision_text=? WHERE id=1",
                    ("To be East Africa's most trusted industrial chemical supplier — recognised for product "
                     "integrity, dependable logistics, and genuine partnership with the businesses we serve "
                     "across the region.",)
                )
            _conn.commit()
        except Exception as _copy_err:
            print(f"[STARTUP] About/Mission/Vision seed warning: {_copy_err}")
